...
function Home() {
  ...
  const handleMagicButtonClick = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:3000/scrape', {
        url: inputLink // Include the URL in the request body
      });
      if (data?.data?.response?.approaches && data.data.response.summary) {
        navigate('/options', {
          state: {
            options: data.data.response.approaches,
            summary: data.data.response.summary,
            url: inputLink,
          },
        });
      }
